...
export const GraphQLEditor = () => {
  ...
  const [variablesValidationError, setVariablesValidationError] = useState<string | null>(null);
...
# }`}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {graphqlVariables.trim() !== "" && graphqlVariables.trim() !== "{}" && (
  ...
  "input": {
    ...
    "email": "john@example.com"
        ...
        <button
          ...
          title={
            variablesValidationError ? "Fix JSON errors before formatting" : "Format Variables JSON"
          }
          className={clsx(
            "mt-2 py-2 rounded text-xs font-semibold cursor-pointer flex items-center gap-2",
            theme === "dark" ? "text-gray-400" : "text-gray-600",
            (isFormattingVariables || variablesValidationError) && "opacity-50 cursor-not-allowed"
          )}
        >
          {isFormattingVariables && <Loader2 className="w-3 h-3 animate-spin" />}
